...
const CoursesCard = ({ course }) => {
  const { currency, getStars } = useContext(AppContext)
  ...
  return (
    <Link
      ...
      <div className="p-3 flex items-start flex-col">
        <h3 className="font-semibold text-base md:text-lg leading-tight text-start">{course.title}</h3>
        ...
        <div className="flex items-start text-xs sm:text-sm justify-center space-x-1 mb-1">
          <span className="font-semibold text-xs sm:text-sm">{course.rating}</span>
          <div className="flex">{getStars(course.rating)}</div>
          <span className="text-gray-400">({course.reviews})</span>
        </div>
        <p className="font-semibold text-sm sm:text-base md:text-lg">{currency}{(course.price - course.discount * course.price / 100).toFixed(2)}</p>
      </div>
    </Link>
  );
};
